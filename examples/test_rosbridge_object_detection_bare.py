"""
Bare rosbridge color-only object detection viewer.

This version avoids `ObjectTracker` entirely and calls Gemini directly through
`src.perception.gemini_object_detection`.

Usage:
    uv run examples/test_rosbridge_object_detection_bare.py
"""
...
from src.perception.gemini_object_detection import detect_objects_in_rgb_array
    ...
    try:
        while ros.is_connected:
            ...
            if should_detect:
                ...
                cv2.imshow("rosbridge_object_detection_bare", raw_bgr)
                cv2.waitKey(1)

                try:
                    detections = detect_objects_in_rgb_array(
                        frame,
                        api_key=api_key,
                        max_objects=MAX_OBJECTS,
                    )
                    print(
                        f"[t={stamp:.2f}] detected {len(detections)} object(s): "
                        f"{[det.label for det in detections]}"
                    )
                    display_frame = cv2.cvtColor(
                        _draw_detections(frame, detections), cv2.COLOR_RGB2BGR
                    )
                except Exception as exc:
            ...
            if display_frame is not None:
                cv2.imshow("rosbridge_object_detection_bare", display_frame)
